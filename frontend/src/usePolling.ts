...
// showing a wrong partition would be worse than showing none. Like
// usePollingRefetchable it also returns a refetch() callback, so a mutation
// (e.g. an Arm/Disarm toggle) can pull the fresh snapshot immediately rather
  ...
  const tick = () => {
    fetcherRef
      ...
      .catch(() => {
        ...
        if (!cancelledRef.current) setValue(null);
      });
  };
