...
// outbound fetch (the robot never shows, and in a later slice never merges on,
// stale authored data — ADR 0016).
//
// Partition invariant (CONTEXT "Outbound"): the six stage lists partition the
// raw authored pull, so
//
//	Outgoing = len(Draft) + len(Red) + len(Running)
//	         + len(ChangesRequested) + len(AwaitingApproval) + len(Ready)
//
...
// rebuildOutbound replaces the outbound snapshot from one authored-PR list
// call (the cycle's second pull). It runs at the tail of EVERY cycle,
// independent of the inbound fetch: the two pulls are disjoint by search and
// fail independently. A failed fetch clears the snapshot to the zero value —
// current truth is "unknown", and the robot never acts on stale authored data.
func (e *Engine) rebuildOutbound(ctx context.Context) {
	...
	// ClassifyOutboundStage is the partition), so the six list lengths sum to
	// Outgoing by construction.
	ob := Outbound{Outgoing: len(authored)}
	for _, pr := range authored {
		item := outboundItem(pr)
		switch github.ClassifyOutboundStage(pr) {
...
// outboundItem projects one authored PR into its stage-list item.
func outboundItem(pr github.PR) OutboundItem {
	return OutboundItem{
		Number:       pr.Number,
		Title:        pr.Title,
		Author:       pr.Author,
