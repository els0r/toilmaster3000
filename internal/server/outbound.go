...
func outboundItemToBody(it engine.OutboundItem, armed bool) OutboundItem {
	return OutboundItem{
		...
		Conflict:          it.Mergeable == forge.MergeableConflicting,
...
// forge.OutboundStages() — make check cannot see a MISSING field, since
...
func outboundToBody(ob engine.Outbound, armed map[int]bool) Outbound {
	return Outbound{
		...
		Draft:            outboundItemsToBody(ob[forge.OutboundStageDraft], armed),
		Red:              outboundItemsToBody(ob[forge.OutboundStageRed], armed),
		Running:          outboundItemsToBody(ob[forge.OutboundStageRunning], armed),
		ChangesRequested: outboundItemsToBody(ob[forge.OutboundStageChangesRequested], armed),
		AwaitingApproval: outboundItemsToBody(ob[forge.OutboundStageAwaitingApproval], armed),
		InDiscussion:     outboundItemsToBody(ob[forge.OutboundStageInDiscussion], armed),
		Ready:            outboundItemsToBody(ob[forge.OutboundStageReady], armed),
		Distribution: OutboundDistribution{
			Draft:            len(ob[forge.OutboundStageDraft]),
			Red:              len(ob[forge.OutboundStageRed]),
			Running:          len(ob[forge.OutboundStageRunning]),
			ChangesRequested: len(ob[forge.OutboundStageChangesRequested]),
			AwaitingApproval: len(ob[forge.OutboundStageAwaitingApproval]),
			InDiscussion:     len(ob[forge.OutboundStageInDiscussion]),
			Ready:            len(ob[forge.OutboundStageReady]),
		},
	}
}
