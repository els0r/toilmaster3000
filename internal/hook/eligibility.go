...
// forge's CLI, today's implicit grant unchanged.
func (r Requires) Grant(active Forge) []string {
	grant := make([]string, 0, len(r.Tools)+1)
	grant = append(grant, forgeCLI[active])
...
// active instance: a different Forge, or the other forge's CLI named
// explicitly in Tools — self-describing without Forge spelled out (ADR 0031
// decision 4). Returns the human reason for the boot log.
func (r Requires) ineligible(active Forge) (string, bool) {
	if r.Forge != "" && r.Forge != active {
		return fmt.Sprintf("scoped to forge %q, this instance runs %q", r.Forge, active), true
	}
	for _, forge := range []Forge{GitHub, GitLab} {
		if forge == active {
			continue
		}
		cli := forgeCLI[forge]
