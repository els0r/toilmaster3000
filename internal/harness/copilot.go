...
func copilotInvoke(ctx context.Context, model, prompt string) ([]byte, error) {
	return runCopilot(ctx, model, prompt, "--available-tools=__none__")
}
...
// (ADR 0023).
func copilotActInvoke(ctx context.Context, model, prompt string) ([]byte, error) {
	return runCopilot(ctx, model, prompt, "--allow-tool", "shell(gh:*)")
}

// runCopilot runs one headless copilot CLI pass. Unlike claude, copilot does
// not attach piped stdin (verified on v1.0.78), so the composed prompt rides
// argv — a prompt beyond the OS argv limit fails exec, a failed attempt on
// the 3-strikes path, never a truncated judgement (ADR 0024). Silent mode
// makes stdout exactly the agent's response text, and the hermetic base flags
// keep the run free of ambient workstation state: no AGENTS.md custom
// instructions, no built-in GitHub MCP, no mid-run self-update.
// CommandContext kills the CLI on cancellation (the hook's timeout), so no
// run outlives its bound.
func runCopilot(ctx context.Context, model, prompt string, extraArgs ...string) ([]byte, error) {
